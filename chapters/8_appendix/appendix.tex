\begin{appendices}
    ...
    \begin{algorithm}
        ...
        \While{$\text{training}$}{
            ...
            \textit{metrics} $\gets$ \text{validate(\textit{student, valSet})}\;
            \text{validate(\textit{teacher, valSet})}\;
            \If {$\textit{metrics.accuracy}$ \text{is increased}}{
    ...
    \begin{algorithm}
        ...
        \For{sample $\gets$ \textit{ulSet}}{
            ...
            \If{(\textit{label=Safe}) \textbf{and} (\textit{S.count \textless S.max})}{
                \textit{trainSet} $\gets$ (\textit{sample, label})\;
